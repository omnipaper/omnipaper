...
export type SaveAiInput = {
  providers: InferRequestType<typeof api.settings.providers.$put>["json"];
  ai: { provider: AiProviderId; model: string };
};

// Save the provider keys plus the AI provider/model (two calls — keys first, then settings), then
// refresh both cards. Mirrors useSaveOcrSettings.
...
export type TestAiInput = InferRequestType<typeof api.settings.ai.test.$post>["json"];

// Dry-run an AI provider key. Like the storage/provider tests, a checked result drives the toast.
export function useTestAiConnection() {
  return useMutation({
    mutationFn: async (input: TestAiInput) => {
      const res = await api.settings.ai.test.$post({ json: input });
      if (!res.ok) {
        throw new Error("Test failed");
      }
      return res.json();
