      ...
      <div className="p-1">
        {PRESETS.map((preset) => (
          <button
            ...
            type="button"
            onClick={() =>
              onChange({ kind: "dateRange", gte: presetGte(preset.unit, preset.amount) })
            }
      ...
      <div className="flex flex-col gap-2 p-2">
        <label className="flex items-center justify-between gap-2 text-muted-foreground text-xs">
          ...
          <input
            ...
            value={range?.gte ?? ""}
            max={range?.lte}
            onChange={(event) => update("gte", event.target.value)}
            className="rounded-md border bg-transparent px-2 py-1 text-foreground text-xs"
          />
        </label>
        <label className="flex items-center justify-between gap-2 text-muted-foreground text-xs">
          <span>To</span>
          <input
            ...
            value={range?.lte ?? ""}
            min={range?.gte}
            onChange={(event) => update("lte", event.target.value)}
            className="rounded-md border bg-transparent px-2 py-1 text-foreground text-xs"
          />
        </label>
      </div>
