...
export function ActiveFilterChip({ field, value, onChange, onRemove, }: {
    ...
    return (<span className="inline-flex items-center rounded-full border border-border bg-muted text-xs">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button type="button" className="flex items-center gap-1.5 rounded-l-full py-1 pr-1.5 pl-2.5 hover:bg-accent">
        ...
        <PopoverContent align="start" className="w-56 p-0">
          {field.picker.kind === "dateRange" ? (<DateFilterPicker value={value} onChange={onChange}/>) : (<FilterValueList options={field.picker.options} selected={selected} onToggle={toggle}/>)}
        </PopoverContent>
      </Popover>
      <button type="button" onClick={onRemove} aria-label={`Remove ${field.label} filter`} className="rounded-r-full py-1 pr-2 pl-1 text-muted-foreground hover:bg-foreground/10 hover:text-foreground">
