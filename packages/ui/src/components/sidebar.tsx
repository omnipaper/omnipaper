...
import {
  ...
  SheetHeader,
  SheetTitle,
} from "@omnipaper/ui/components/sheet";
import { Skeleton } from "@omnipaper/ui/components/skeleton";
import { Tooltip, TooltipContent, TooltipTrigger } from "@omnipaper/ui/components/tooltip";
import { useIsMobile } from "@omnipaper/ui/hooks/use-mobile";
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium uppercase tracking-wider text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className,
      )}
      {...props}
    />
